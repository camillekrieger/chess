...
public class GamePlayUI {
    ...
    private void drawBlackLegal(PrintStream out, Collection<ChessPosition> positions, ChessPosition start, ChessGame currGame){
        ...
        for (int squareRow = 1; squareRow <= GAME_BOARD_DIMENSIONS; squareRow++) {
            ...
            for (int boardCol = 8; boardCol > 0; boardCol--) {
                legalMove = legalMove(positions, squareRow, boardCol);
                boardColor = getBoardColor(boardColor, legalMove, squareRow, boardCol, start, currGame, out);
            }
    ...
    private boolean legalMove(Collection<ChessPosition> positions, int squareRow, int boardCol){
        boolean legalMove = false;
